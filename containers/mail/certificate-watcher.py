...
class CertificateWatcher:
    def __init__(self):
        ...
        logger.info(f"Certificate watcher initialized - domain: {self.mail_domain}, preference: {self.cert_type_preference}")
        ...
        try:
            with self.db_connection.cursor() as cur:
                ...
                if result:
                    ...
                    logger.debug(f"Current certificate status - type: {cert_type}, ssl: {ssl_enabled}, updated: {last_updated}")
        ...
        try:
            with self.db_connection.cursor() as cur:
                ...
                if best_priority > current_priority:
                    logger.info(f"Better certificate available: {best_cert_type} (priority {best_priority}) > {self.current_cert_type} (priority {current_priority})")

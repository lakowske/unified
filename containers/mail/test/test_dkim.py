...
import smtplib
import imaplib
from containers.mail.test.utils import search_emails_by_subject
...
class TestDKIMFunctionality:
    ...
    def test_dkim_postfix_integration(self, mail_config):
        ...
        try:
            ...
            result = subprocess.run([
                'podman', 'run', '--rm', 
                '-e', f'MAIL_DOMAIN={mail_config["mail_domain"]}',
                'unified/mail:latest', 
                'bash', '-c',
                '''
            ...
            assert "non_smtpd_milters = inet:localhost:8891" in config_output, "Postfix should use OpenDKIM for non-SMTP"
...
class TestDKIMIntegration:
    ...
    def test_mail_container_with_dkim(self, mail_config):
        ...
        try:
            ...
            # Cleanup
            subprocess.run(['podman', 'stop', 'test-mail-dkim'], 
                         capture_output=True, timeout=10)
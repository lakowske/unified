...
class MailboxManager:
    ...
    def __init__(self, mail_base_dir: str = "/var/mail", vmail_uid: int = 5000, vmail_gid: int = 5000):
    ...
    def update_mailbox(self, old_domain: str, old_username: str, new_domain: str, new_username: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to update mailbox - from: {old_domain}/{old_username}, to: {new_domain}/{new_username}, error: {str(e)}")
            return False
    ...
    def _set_ownership_recursive(self, path: Path):
        ...
        try:
            ...
            subprocess.run([
                "chown", "-R", f"{self.vmail_uid}:{self.vmail_gid}", str(path)
...
class DatabaseListener:
    ...
    def __init__(self, db_config: Dict[str, str], mailbox_manager: MailboxManager):
    ...
    def _process_notification(self, channel: str, payload: str):
        ...
        try:
            ...
            logger.error(f"Failed to parse notification payload - channel: {channel}, payload: {payload}, error: {str(e)}")
        except Exception as e:
    ...
    def _handle_user_updated(self, data: Dict):
        ...
        if success:
            logger.info(f"User update processed successfully - from: {old_domain}/{old_username}, to: {new_domain}/{new_username}")
        else:
            logger.error(f"Failed to process user update - from: {old_domain}/{old_username}, to: {new_domain}/{new_username}")
...
def main():
    ...
    logger.info(f"Configuration - db_host: {db_config['host']}, db_port: {db_config['port']}, db_name: {db_config['dbname']}")

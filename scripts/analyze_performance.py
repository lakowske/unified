...
class PerformanceAnalyzer:
    ...
    def _generate_startup_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if avg_startup:
            if avg_startup > 120:
                recommendations.append(f"Overall average startup time is high ({avg_startup:.1f}s). Consider optimizing slow containers.")
            elif avg_startup > 60:
                recommendations.append(f"Overall average startup time is moderate ({avg_startup:.1f}s). Room for improvement.")
        ...
        if slowest_containers:
            for container in slowest_containers[:3]:
                if container["startup_time"] > 60:
                    recommendations.append(f"Container {container['name']} is slow ({container['startup_time']:.1f}s startup)")
    ...
    def compare_environments(self) -> Dict[str, Any]:
        ...
        if valid_envs:
            ...
            comparison["performance_rankings"]["by_startup_time"] = [
                {"environment": env, "startup_time": metrics["startup_time"]} 
                for env, metrics in sorted_by_startup
            ]
    ...
    def generate_optimization_report(self) -> str:
        ...
        if overall_stats:
            ...
            report_lines.append(f"Startup Time Range: {overall_stats['min_startup_time']:.2f}s - {overall_stats['max_startup_time']:.2f}s")
        ...
        if env_analysis:
            ...
            for env_name, env_data in env_analysis.items():
                ...
                status = "🔴" if startup_time and startup_time > 90 else "🟡" if startup_time and startup_time > 60 else "🟢"
        ...
        if container_analysis:
            ...
            if slowest:
                ...
                for i, container in enumerate(slowest[:5], 1):
                    report_lines.append(f"  {i}. {container['name']} ({container['environment']}): {container['startup_time']:.2f}s")
                report_lines.append("")
...
Examples:
  ...
  python scripts/analyze_performance.py --compare
    ...
    parser.add_argument(
        "--file",
        type=str,
        help="Specific performance data file to analyze"
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Analyze trends from historical files"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    print(f"Performance Data Analyzer")
    ...
    try:
        ...
        if not any([args.report, args.compare, args.trends, args.export]):
            ...
            if overall_stats:
                ...
                print(f"  Performance Range: {overall_stats['min_startup_time']:.2f}s - {overall_stats['max_startup_time']:.2f}s")

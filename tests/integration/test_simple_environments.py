...
class TestSimpleEnvironments:
        ...
        for env_name in ["test-env-1", "test-env-2"]:
            ...
            if env_name in self.expected_ports["environments"]:
                ...
                if "postgres" in expected_data["ports"]:
                    ...
                    assert actual_port == expected_port, f"Database port mismatch: expected {expected_port}, got {actual_port}"
    ...
    def test_network_port_queries(self) -> None:
        ...
        assert port_mapping.startswith("5001:"), f"Expected postgres port mapping to start with '5001:', got {port_mapping}"
        ...
        assert apache_port_mapping.startswith("8001:"), f"Expected apache port mapping to start with '8001:', got {apache_port_mapping}"
        ...
        assert bind_port_mapping.startswith("5301:"), f"Expected bind port mapping to start with '5301:', got {bind_port_mapping}"

...
class TestContainerPerformance:
    ...
    def setup_method(self):
        ...
        self.runner.configure_test(test_iterations=2, warmup_iterations=1, startup_timeout=180, cooldown_time=5)
    ...
    def test_container_health_check_performance(self):
        ...
        for container_name, healthy_time in result["healthy_times"].items():
            ...
            assert (
                healthy_time < 120
            ), f"Container {container_name} took too long to become healthy: {healthy_time:.2f}s"
    ...
    def test_startup_performance_consistency(self):
        ...
        results = self.runner.run_environment_performance_test(environment, iterations=3, include_warmup=True)
    ...
    def test_environment_comparison(self):
        ...
        for env in environments:
            results = self.runner.run_environment_performance_test(env, iterations=2, include_warmup=True)
    ...
    def test_performance_regression_detection(self):
        ...
        # Run another test
        results2 = self.runner.run_environment_performance_test(environment, iterations=2, include_warmup=True)
    ...
    def test_comprehensive_performance_report(self):
        ...
        for env in environments:
            results = self.runner.run_environment_performance_test(env, iterations=1, include_warmup=False)
            assert "error" not in results
